...
export default function MarshalLogin({ isDemo }: { isDemo: boolean }) {
  ...
  if (!group) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center bg-gray-50 px-6 py-12">
        ...
        {isDemo && (
          <p className="mt-1 text-xs text-orange-600">Demo passcode shown on next screen</p>
        )}
    ...
    <div className="flex min-h-dvh flex-col items-center justify-center bg-gray-50 px-6 py-12">
      ...
      {isDemo && (
        <p className="mt-1 font-mono text-sm text-orange-600">
          Demo passcode: <strong>{DEMO_PASSCODES[group]}</strong>
        </p>
      )}
