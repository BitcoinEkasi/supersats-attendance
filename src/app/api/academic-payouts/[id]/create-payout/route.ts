...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const e of rewarded) {
    if (e.participant.paymentMethod === "LIGHTNING_ADDRESS" && !e.participant.boltUserId) {
      try {
        ...
        console.error(`[academic-create-payout] Bolt account creation failed for ${e.participant.tskId}:`, err.message);
      }
    }
  }
  ...
  const totalSats = eligible.reduce((sum, e) => sum + e.rewardSats, 0);
  const memo = `TSK school grades ${payout.year} T${payout.term}`;
  ...
  try {
    ...
    console.error(`[academic-create-payout] Failed to fetch bolt reserve:`, err.message);
  }
